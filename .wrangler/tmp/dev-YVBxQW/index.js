...
// .wrangler/tmp/bundle-7rbe4h/checked-fetch.js
...
<div class="sheet">
  ...
  <div class="photo-drop" onclick="document.getElementById('photo-inp').click()">
    <div style="font-size:30px;margin-bottom:8px">\u{1F4F7}</div>
...
<div class="photo-viewer" id="photo-viewer">
  ...
  <div class="photo-viewer-footer">
    ...
    <div class="viewer-tag-row">
      <button class="viewer-tag-btn pro-btn" onclick="startTag('pro')">\u{1F44D} Pro</button>
      <button class="viewer-tag-btn con-btn" onclick="startTag('con')">\u{1F44E} Con</button>
    </div>
...
function renderList() {
  ...
  if (!apts.length) return \`<div class="empty"><div class="empty-icon">\u{1F3D9}\uFE0F</div><h3>No apartments yet</h3><p>Tap + and type any address.<br>Live listing data fills in automatically.</p></div>\`;
  ...
  const cards = filtered.map(a => {
    ...
    const thumb = photo ? \`<div class="card-photo"><img src="\${photo}"></div>\` : \`<div class="card-photo">\u{1F3E2}</div>\`;
    ...
    const meta = [a.neighborhood, a.bldg, a.beds?a.beds+'br':null, a.baths?a.baths+'ba':null, a.sqft?a.sqft+'sf':null].filter(Boolean).join(' \xB7 ');
    return \`<div class="apt-card" onclick="openDetail('\${a.id}')">
      ...
      <div class="card-body">
        ...
        <div class="card-bottom">
              ...
              <div style="display:flex;align-items:center;gap:6px">
                \${a.showDate ? \`<span style="font-size:10px;color:var(--muted)">\u{1F4C5} \${new Date(a.showDate+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>\` : ''}
  ...
  }).join('') || \`<div class="empty"><div class="empty-icon">\u{1F50D}</div><p>No units with this status.</p></div>\`;
  return pills + cards;
}
...
function renderDetail() {
  ...
  return \`
    ...
    <div style="color:var(--muted);font-size:12px;margin-bottom:10px">\${[a.neighborhood,a.btype].filter(Boolean).join(' \xB7 ')}</div>
...
function renderCompare() {
  ...
  const rxSummary = (apt, type) => {
    ...
    return items.map(r => \`<div style="font-size:12px;margin-bottom:3px">\${type==='pro'?'\u2713':'\u2717'} \${esc(r.text)}\${r.photoIndex!=null?' \u{1F5BC}':''}</div>\`).join('');
  };
  const rows=[
...
function renderShortlist() {
  ...
  return \`<div style="font-family:'Playfair Display',serif;font-size:18px;margin-bottom:16px">Your Top Picks</div>\`+
    top.map(a=>{
      const stars=[1,2,3,4,5].map(i=>\`<span class="star \${i<=a.rating?'on':''}">\u2605</span>\`).join('');
      const pros=(a.reactions||[]).filter(r=>r.type==='pro').slice(0,2).map(r=>\`\u2713 \${r.text}\`).join(' \xB7 ')||'';
      return \`<div class="apt-card" onclick="openDetail('\${a.id}')"><div class="card-body">
        <div class="card-top"><div><div class="card-name">\${a.name}</div><div class="card-meta">\${[a.neighborhood,a.price].filter(Boolean).join(' \xB7 ')}</div></div><div class="stars">\${stars}</div></div>
...
function renderViewerReactions() {
  ...
  const chips = (a.reactions || [])
    ...
    .map(r => \`<span class="viewer-reaction-chip \${r.type === 'pro' ? 'pro' : 'con'}">
      ...
      <button onclick="deleteReaction('\${esc(r.id)}')">\xD7</button>
    </span>\`).join('');
...
function renderFormReactions() {
  ...
  el.innerHTML = _formReactions.map((r, i) => \`
    <div class="reaction-editor-item">
      ...
      <button type="button" class="reaction-del" onclick="removeFormReaction(\${i})">\xD7</button>
...
__name(handleGetApts, "handleGetApts");
...
__name(handleSaveApts, "handleSaveApts");
var index_default = {
...
// .wrangler/tmp/bundle-7rbe4h/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7rbe4h/middleware-loader.entry.ts
